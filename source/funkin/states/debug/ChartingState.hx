...
class ChartingState extends funkin.backend.MusicBeat.MusicBeatState
{
	...
	final eventColors:Array<FlxColor> = [
		0xFFFFFFFF, 0xFFFF0000, 0xFF00FF00, 0xFF0000FF, 0xFFFF00FF, 0xFFFFFF00, 0xFF00FFFF, 0xFFFF9100, 0xFFA200FF, 0xFFC24B99, 0xFF00FFFF, 0xFF12FA05,
		0xFFF9393F, 0xFF00FFBF, 0xFFFF0095, 0xFFC8FF00, 0xFF0077FF
	];
	...
	**/
	...
	override function create()
	{
		...
		for (iy in 0...4 * gridBG2Length)
		{
			for (ix in 0...3)
			{
				...
				var gridSegment = new FlxSprite(GRID_SIZE * ix * 4,
					GRID_SIZE * iy * 4)
					.loadGraphic((isPurple ? (isDark ? gridDarkPurpleBitmap : gridPurpleBitmap) : (isDark ? gridDarkWhiteBitmap : gridWhiteBitmap)));
	...
	override function update(elapsed:Float)
	{
		...
		if (Startup.hasEe2 && lilBuddiesBox.checked)
		{
				...
				&& FlxG.mouse.justPressed)
			{
				...
				var poop:String = funkin.backend.Highscore.formatSong("lil-buddies", 2);
	...
	function updateHeads(?changedCharacters:Bool = false):Void
	{
		...
		{
			var leftChar = new funkin.objects.Character(0, 0, player2DropDown.selectedLabel);
			var rightChar = new funkin.objects.Character(0, 0, player1DropDown.selectedLabel);
