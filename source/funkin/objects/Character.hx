	...
	override function update(elapsed:Float)
	{
		if (!debugMode)
		{
			...
			switch (curCharacter)
			{
				...
				case "pico-speaker":
					// for pico??
					if (TankmenUnit.animationNotes.length > 0)
					{
						if (Conductor.songPosition > TankmenUnit.animationNotes[0][0])
						{
							// trace('played shoot anim' + TankmenUnit.animationNotes[0][1]);

							var shootAnim:Int = 1;

							if (TankmenUnit.animationNotes[0][1] >= 2)
								shootAnim = 3;

							shootAnim += FlxG.random.int(0, 1);

							playAnim('shoot' + shootAnim, true);
							TankmenUnit.animationNotes.shift();
						}
					}
			}
		}
